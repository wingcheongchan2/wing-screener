...
# 2. TSLA 專屬背景：黑色底 + 紅色 Logo (應您的要求更換)
...
def display_dashboard(row):
    ...
    c1.markdown(f'<div class="stat-card"><div class="stat-label">現價 PRICE</div><div class="stat-value">${row["Close"]:.2f}</div></div>', unsafe_allow_html=True)
    c2.markdown(f'<div class="stat-card" style="border-bottom:3px solid #00E676"><div class="stat-label">買入 ENTRY</div><div class="stat-value" style="color:#00E676">${row["Entry"]:.2f}</div></div>', unsafe_allow_html=True)
    c3.markdown(f'<div class="stat-card" style="border-bottom:3px solid #FF1744"><div class="stat-label">止蝕 STOP</div><div class="stat-value" style="color:#FF1744">${row["Stop"]:.2f}</div></div>', unsafe_allow_html=True)
    c4.markdown(f'<div class="stat-card"><div class="stat-label">目標 TARGET (3R)</div><div class="stat-value">${row["Target"]:.2f}</div></div>', unsafe_allow_html=True)
    ...
    <div class="report-panel">
        ...
        <div style="border-top:1px solid #333; padding-top:15px; color:#aaa; font-size:14px;">
            ...
            1. 設定 <b>Stop Limit Buy Order (觸價買單)</b> 於 <b>${row['Entry']:.2f}</b>。<br>
            2. 成交後，立即設定止損於 <b class="report-risk">${row['Stop']:.2f}</b>。<br>
            3. 預期風險回報比 (R/R) 為 <b>1:{row['RR']:.1f}</b>。
        </div>
    </div>
    """, unsafe_allow_html=True)
...
if mode == "⚡ 強勢股掃描器 (Scanner)":
        ...
        if sel:
            ...
            if not d.empty:
                ...
                if r: display_dashboard(r)
                    ...
                    st.metric("現價", f"${d['Close'].iloc[-1]:.2f}")
    ...
    # 這裡顯示紅色 Tesla Logo 頁面
    st.markdown("<h1 style='text-align:center; color:#fff; text-shadow: 0 0 20px #E53935;'>⚡ TESLA 戰情室</h1>", unsafe_allow_html=True)
    c1,c2,c3 = st.columns(3)
    c1.link_button("Google 新聞", "https://www.google.com/search?q=Tesla+stock&tbm=nws", use_container_width=True)
    c2.link_button("Elon Musk X", "https://twitter.com/elonmusk", use_container_width=True)
    c3.link_button("完整圖表", "https://www.tradingview.com/chart/?symbol=TSLA", use_container_width=True)
    st.divider()
