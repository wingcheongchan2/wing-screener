...
def inject_tesla_theme():
    ...
    <style>
        ...
        .stApp {{
            background: linear-gradient(180deg, rgba(8, 11, 16, 0.88) 0%, rgba(11, 14, 20, 0.95) 100%),
        ...
        div.stButton > button:first-child {{
            background: linear-gradient(135deg, #181c24 0%, #0d1017 100%);
            color: #FFFFFF;
            border: 1px solid #E82127; /* Tesla 經典紅色 */
            border-radius: 4px;
            padding: 10px 24px;
        ...
        div.stButton > button:first-child:active {{
            ...
            box-shadow: 0 0 10px rgba(232, 33, 39, 0.4);
        }}
    ...
    try:
        ...
        if curr_price > sma50 and sma50 > sma150 and sma150 > sma200:
            ...
            if dist_h52 >= -25.0 and dist_l52 >= 25.0:
                score += 5
                is_stage2 = True
                reasons.append("Stage 2 均線完美多頭排列")
        ...
        if rs_rating >= 80:
            score += 25
            reasons.append(f"強勢領頭羊 (RS {rs_rating} 跑贏大市)")
        elif rs_rating >= 65:
            score += 18
        ...
        if (atr14 / (atr50 if atr50 > 0 else 1)) <= 0.85:
            score += 10
            reasons.append("VCP 波動收窄 (波幅收斂蓄勢)")
...
# 左側導航欄：移除複雜計算，保持簡潔極致的 Tesla 終端風格
with st.sidebar:
    st.markdown("## ⚡ TESLA CYBER TERMINAL")
    st.caption("J Law Alpha Hunter • 旗艦版")
    st.markdown("---")
    ...
    st.caption(f"全網監測標的總數：**{len(full_scan_list)}** 隻")
    
    st.write("")
    if st.button("⚡ 重新掃描市場 (SCAN MARKET)"):
        st.session_state.pop('scan_data', None)
        st.rerun()

    st.markdown("---")
    st.markdown("""
    <div style="font-size:12px; color:#94A3B8; font-family:'JetBrains Mono'; line-height:1.6;">
        <b>系統規則架構:</b><br>
        • Stage 2 範式過濾<br>
        • 加權百分位 RS 強度<br>
        • VCP 波動收窄蓄勢<br>
...
if total_pos_cost > max_allowed_cost:
    ...
    total_pos_cost = calc_shares * target_entry
    pos_pct_of_capital = (total_pos_cost / account_capital) * 100
    adjusted_warning = f"⚠️ 提示：推薦股數受限於單一持倉上限 ({max_pos_cap}%)，已自動調整為安全股數。"
...
with plan_c1:
    ...
    <div class="action-box">
        <h4 style="margin:0 0 14px 0; color:#38BDF8;">🎯 {selected_stock} 精確進出場點位</h4>
        <div style="display:grid; grid-template-columns: repeat(2, 1fr); gap: 14px; font-size:15px;">
            <div style="background:rgba(255,255,255,0.04); padding:12px; border-radius:6px;">
                ...
                <b style="font-size:22px; color:#38BDF8;">${target_entry:.2f}</b>
            </div>
            <div style="background:rgba(255,255,255,0.04); padding:12px; border-radius:6px;">
                <span style="color:#94A3B8; font-size:12px;">🔴 防守止損點 (Stop Loss):</span><br>
                <b style="font-size:22px; color:#EF4444;">${target_stop:.2f} ({target_stop_pct}%)</b>
            </div>
            <div style="background:rgba(255,255,255,0.04); padding:12px; border-radius:6px;">
...
with plan_c2:
    ...
    <div class="action-box" style="border-color: rgba(16, 185, 129, 0.4);">
        <h4 style="margin:0 0 14px 0; color:#10B981;">🛡️ J Law 1% 風險倉位計算結果</h4>
        <div style="line-height:2.0; font-size:14px;">
            <div>• 總賬戶資金: <b>${account_capital:,.2f}</b></div>
            <div>• 單筆最大虧損 (1R): <b style="color:#EF4444;">${max_risk_dollars:,.2f}</b> ({risk_pct}%)</div>
            <div>• 每股承受風險: <b>${target_risk_per_share:.2f}</b></div>
            <hr style="border:0; border-top:1px solid rgba(255,255,255,0.1); margin:10px 0;">
            <div>• <b>推薦下單股數:</b> <span style="font-size:24px; color:#10B981; font-weight:800;">{calc_shares} 股</span></div>
            <div>• <b>總持倉金額:</b> <b>${total_pos_cost:,.2f}</b> ({pos_pct_of_capital:.1f}% 倉位)</div>
            <div>• <b>觸發止損時總虧損:</b> <b style="color:#EF4444;">-${calc_shares * target_risk_per_share:,.2f}</b> (嚴格控制在 1R 內)</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
